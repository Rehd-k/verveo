...
export default function AdminRetailerDetailPage() {
  ...
  if (!retailer) return <div className="text-text-secondary">Loading...</div>;
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-xl border border-white/10 bg-card-dark p-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <h2 className="text-xl font-bold text-white">{retailer.businessName}</h2>
            <p className="text-text-secondary">{retailer.venueType}</p>
            {retailer.user && (
            ...
            {retailer.address && (
              <p className="mt-2 text-sm text-text-secondary">{retailer.address}</p>
            )}
          </div>
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          <div>
            ...
            <div className="mt-1 flex gap-2">
              ...
              <button
                ...
                className="rounded-lg bg-primary px-3 py-2 text-sm font-semibold text-background-dark"
              >
                Save
              </button>
            </div>
          ...
          <div>
            <label className="text-xs text-text-secondary">Venue type</label>
            <div className="mt-1 flex gap-2">
              <select
                value={venueType}
          ...
          <div>
            <label className="text-xs text-text-secondary">Allowance</label>
            <div className="mt-1 flex gap-2">
              <input
                type="number"
          ...
          <div>
            <label className="text-xs text-text-secondary">Current Stock</label>
            <div className="mt-1 flex gap-2">
              <input
                type="number"
      ...
      <div className="rounded-xl border border-white/10 bg-card-dark p-6">
        <div className="flex items-center justify-between">
          <h3 className="font-semibold text-white">Pending stock orders</h3>
          <Link href="/admin/stock-orders" className="text-sm text-primary hover:underline">
            View all
          </Link>
        </div>
        {orders.length === 0 ? (
          <p className="mt-3 text-sm text-text-secondary">No pending stock orders.</p>
        ) : (
            ...
            {orders.map((order) => (
              ...
              >
                <div>
                  ...
                  <p className="text-xs text-text-secondary">{order.notes || 'No notes'}</p>
                </div>
      ...
      <div className="rounded-xl border border-white/10 bg-card-dark p-6">
        <div className="flex items-center justify-between">
          <h3 className="font-semibold text-white">Proofs</h3>
          <Link href="/admin/proofs" className="text-sm text-primary hover:underline">
            Review queue
          </Link>
        </div>
        {proofs.length === 0 ? (
          <p className="mt-3 text-sm text-text-secondary">No proofs submitted yet.</p>
        ) : (
          <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {proofs.slice(0, 6).map((proof) => (
              <div key={proof.id || proof._id} className="overflow-hidden rounded-lg border border-white/10">
                <div
