    ...
    <div className="flex-1 relative flex flex-col bg-background-dark md:h-[91.8vh] h-[80vh] w-full">
      <div className="flex flex-1 overflow-hidden">
        <main className="flex-1 relative">
          ...
          <div className="absolute left-2 right-2 top-14 md:top-1.5 md:right-2 md:left-auto bg-black/70 text-white p-3 md:p-4 rounded-lg z-10">
          ...
          <button
            className="md:hidden absolute bottom-6 left-2 h-12 w-12 rounded-full bg-background-dark border border-border-dark text-gray-400 hover:text-primary hover:bg-white/5 transition-all flex items-center justify-center group"
          ...
          <section
            className={`absolute bottom-1.5 md:top-1.5 left-2 md:w-85 w-[96%] md:h-120 h-0 flex flex-col z-20 bg-background-dark rounded-lg ${showUserMenu ? 'h-170' : ''}`}
          >
            <div className="p-2 flex items-center justify-between gap-3">
              <div>
                ...
                <p className="text-xs font-bold text-white mt-1">Location Targeting</p>
              </div>
              <button
            ...
            <div className="flex-1 overflow-y-auto px-6 space-y-6">
              <div className="space-y-3">
                ...
                <input
                  ...
                  className="w-full px-3 py-2 rounded-lg bg-white/5 border border-border-dark text-xs text-white placeholder:text-gray-500 focus:outline-none focus:border-primary/40"
                />
                <div className="flex flex-wrap gap-2 max-h-32 overflow-y-auto">
                  {filteredCities.map((city) => (
                    <button
                      ...
                      className="px-3 py-1.5 rounded-full text-xs font-medium transition-all border bg-white/5 text-gray-400 border-border-dark hover:border-primary/40 hover:text-primary"
                    >
                      {city.name}
                    </button>
                  ))}
                  ...
                  {VENUE_CATEGORIES.map((category) => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-xs font-medium text-white">{CATEGORY_LABELS[category].title}</p>
                          <p className="text-xs text-gray-500">{CATEGORY_LABELS[category].subtitle}</p>
                        </div>
                      </div>
                    </label>
                  ))}
              ...
              <div className="space-y-4 border-t border-border-dark py-4">
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-xs font-medium text-gray-200">High Foot Traffic</p>
                    <p className="text-[11px] text-gray-500">Focus on premium volume locations</p>
                  </div>
      ...
      <footer className="min-h-14 border-t border-gray-500 flex items-center justify-between md:px-12 px-3 py-2 z-30 shrink-0 gap-2">
        <div className="flex items-center md:gap-10 gap-2 min-w-0">
          <button
            type="button"
            className="md:hidden shrink-0 text-[10px] font-semibold text-gray-400 hover:text-white transition-colors px-1"
          >
            Save Draft
          </button>
          <div className="flex flex-col min-w-0">
            <span className="text-gray-500 text-[9px] sm:text-[10px] font-bold uppercase tracking-widest sm:tracking-[0.15em] mb-0.5 sm:mb-1">
              Est. Audience Reach
            </span>
            <div className="flex items-baseline gap-1 sm:gap-2">
              <span className="text-white text-xs sm:text-sm font-bold tracking-tight transition-all duration-300">
          ...
          <div className="flex flex-col min-w-0">
            ...
            <div className="flex items-baseline gap-1 sm:gap-2">
              ...
              <span className="text-gray-400 text-xs sm:text-sm font-medium">venues</span>
            </div>
          </div>
        </div>
        ...
        <button
          disabled={!canProceed}
          onClick={nextStage}
          className="px-4 py-2 cursor-pointer rounded-xl bg-primary text-background-dark hover:bg-primary/90 font-bold text-xs shadow-[0_8px_30px_rgb(242,208,13,0.15)] transition-all flex items-center gap-2 group md:hidden disabled:opacity-40 disabled:cursor-not-allowed shrink-0"
        >
          Next
        ...
        <div className="md:flex items-center gap-4 hidden">
          ...
          <button
            onClick={nextStage}
            disabled={!canProceed}
            className="px-5 py-2 cursor-pointer rounded-xl bg-primary text-background-dark hover:bg-primary/90 font-bold text-xs shadow-[0_8px_30px_rgb(242,208,13,0.15)] transition-all flex items-center gap-3 group disabled:opacity-40 disabled:cursor-not-allowed"
          >
